document.addEventListener('DOMContentLoaded', () => {
    ...
    function getColumnValues() {
        ...
        allFilters.forEach(filter => {
            ...
            const colIndexNumber = Number(colIndex); // Convert column index to a number
    ...
    function updateSelectOptions(unique_col_values) {
        ...
        allFilters.forEach(filter => {
            ...
            if (unique_col_values[colIndex]) {
                let optionsHTML = '<option value="all">All</option>'; // Default option for 'All'
    ...
    function filter_rows() {
        ...
        allFilters.forEach(filter => {
            const colIndex = filter.getAttribute('data-col-index'); // Get column index from filter
            const value = filter.value; // Get selected value from filter
            if (value !== "all") {
        ...
        rows.forEach(row => {
            ...
            if (display_row && dateFilter.value) {
                const dateColIndex = dateFilter.getAttribute('data-col-index'); // Get column index for the date
    ...
    const allFilters = document.querySelectorAll(".table-filter"); // Select all filter elements
    allFilters.forEach(filter => {
        filter.addEventListener('change', filter_rows); // Add event listener to filter elements
    });

    const dateFilter = document.querySelector(".date-input"); // Select date filter input
    dateFilter.addEventListener('change', filter_rows); // Add event listener to date filter input
});
